...
CREATE SQL SECURITY INVOKER VIEW waits_by_user_by_latency AS
SELECT user,
       event_name AS event,
...
DROP VIEW IF EXISTS waits_by_user_by_latency_raw;

CREATE SQL SECURITY INVOKER VIEW waits_by_user_by_latency_raw AS
SELECT user,
       event_name AS event,
       count_star AS total_events,

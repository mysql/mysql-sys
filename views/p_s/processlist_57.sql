...
SELECT pps.thread_id AS thd_id,
       ...
       ps_helper.format_statement(pps.processlist_info) AS current_statement,
       ps_helper.format_time(esc.lock_time) AS lock_latency,
       esc.rows_examined,
       esc.rows_sent,
       esc.rows_affected,
       ...
       IF(esc.no_good_index_used > 0 OR esc.no_index_used > 0, 
          'YES', 'NO') AS full_scan,
       ps_helper.format_bytes(SUM(mem.current_number_of_bytes_used)) AS current_memory,
       IF(esc.timer_wait IS NOT NULL,
          ps_helper.format_statement(esc.sql_text),
          NULL) AS last_statement,
       IF(esc.timer_wait IS NOT NULL,
          ps_helper.format_time(esc.timer_wait),
          NULL) as last_statement_latency,
       ewc.event_name AS last_wait,
       IF(ewc.timer_wait IS NULL AND ewc.event_name IS NOT NULL, 
          'Still Waiting', 
          ps_helper.format_time(ewc.timer_wait)) last_wait_latency,
       ewc.source
